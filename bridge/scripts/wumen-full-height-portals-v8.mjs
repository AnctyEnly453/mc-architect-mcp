...
const operations = [];
for (const [x1, x2] of [[724, 736], [773, 787], [824, 836]]) {
  operations.push({
    block: "minecraft:air",
    from: { x: x1, y: 169, z: 497 },
    to: { x: x2, y: 190, z: 509 },
  });
  operations.push({
    block: "minecraft:dark_oak_log[axis=x]",
    from: { x: x1 - 2, y: 191, z: 507 },
    to: { x: x2 + 2, y: 193, z: 509 },
  });
}
...
const response = await fetch(`http://127.0.0.1:${config.port}/v1/apply`, {
  ...
  body: JSON.stringify({ operations, label: "Meridian Gate full-height portals v8", projectId: "forbidden-city-wumen-facade-v5", dryRun: !apply }),
});
